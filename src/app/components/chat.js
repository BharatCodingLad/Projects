...
const themes = {
  princess: {
    ...
    buttonGradient: 'from-purple-400 to-pink-400 hover:from-purple-500 hover:to-pink-500',
  ...
  goth: {
    ...
    buttonGradient: 'from-purple-800 to-gray-900 hover:from-purple-900 hover:to-black',
...
export default function Chat() {
  ...
  return (
    <div className={`fixed inset-0 ${themes[theme].background} transition-colors duration-500`}>
      ...
      <div className="h-screen max-w-4xl mx-auto p-4 flex flex-col">
        ...
        <button
          ...
          title={theme === 'princess' ? 'Switch to Goth Mode' : 'Switch to Princess Mode'}
        >
          {theme === 'princess' ? '🖤' : '👑'}
        </button>
        ...
        <div className="h-[15vh] flex flex-col justify-center items-center">
          <h1 className="text-3xl sm:text-4xl font-bold text-center">
            <span className={`bg-gradient-to-r ${themes[theme].gradientText} text-transparent bg-clip-text font-serif`}>
              {theme === 'princess' ? '✨ Princess Chat Palace ✨' : '🖤 Gothic Realm Chat 🖤'}
            </span>
          </h1>
          
          <p className={`text-center ${themes[theme].text} mt-2 italic font-medium text-sm sm:text-base`}>
            {theme === 'princess' 
        ...
        {/* Chat Container */}
        <div className={`h-[70vh] relative ${themes[theme].chatBg} backdrop-blur-sm 
              ...
              <div
                ...
                className={`relative p-3 sm:p-4 my-2 sm:my-3 rounded-2xl 
                           ${msg.role === 'user'
                             ...
                             : `mr-auto ${themes[theme].chatBg} text-${theme === 'princess' ? 'gray-800' : 'gray-200'} max-w-[90%] sm:max-w-[85%] border ${themes[theme].border} shadow-sm`
                           }`}
              >
                ...
                <div className="flex justify-between items-center mb-1 sm:mb-2 text-xs sm:text-sm">
                  <span className="font-medium flex items-center gap-1 backdrop-blur-sm px-2 py-0.5 rounded-full">
                    {msg.role === 'user' 
                      ? <>👑 <span className="hidden sm:inline font-serif">Royal You</span></>
                      : <>🌟 <span className="hidden sm:inline font-serif">Enchanted Assistant</span></>
                    }
                  </span>
                  <span className="opacity-75 text-xs">{formatTime(msg.timestamp)}</span>
                </div>
        ...
        <div className="h-[15vh] flex flex-col justify-center gap-2">
          <textarea
            ...
            className={`w-full p-3 ${themes[theme].chatBg} backdrop-blur-sm 
                     text-${theme === 'princess' ? 'gray-800' : 'gray-200'}
                     border-2 ${themes[theme].border} rounded-2xl
                     focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent
                     placeholder-gray-400 resize-none h-16
                     font-medium transition-colors duration-500`}
            placeholder={theme === 'princess' ? '✨ Share your wishes and dreams...' : '🖤 Whisper your dark thoughts...'}
            disabled={isLoading}
          />
          <div className="flex justify-between items-center">
            <span className={`text-xs sm:text-sm ${themes[theme].text} font-medium italic`}>
              {theme === 'princess' ? '✨ Sprinkle some magical markdown ✨' : '🖤 Cast your markdown spells 🖤'}
            </span>
            <button
              onClick={sendMessage}
              className={`px-6 py-2.5 bg-gradient-to-r ${themes[theme].buttonGradient}
            ...
            >
              {isLoading 
                ? (theme === 'princess' ? '✨ Casting Spell...' : '🖤 Summoning...') 
                : (theme === 'princess' ? '✨ Send Wishes ✨' : '🖤 Cast Message 🖤')}
            </button>
          </div>
        </div>
      </div>
